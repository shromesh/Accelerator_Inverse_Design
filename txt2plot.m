...
output_folder_name = 'result/plot_feb17';
...
%% 4. ダブルチャネルの派生量 (Power Equivalent Value)
...
%----------------------------------------------------------
% (A) 「for each gap_gap」(複数線) [ダブルチャネルのみ]
%    -> 複数線なので色はデフォルトのまま (区別用)。
%       レジェンドの順番を変えたい場合はここでflipするなど。
%----------------------------------------------------------
function plot_with_nan_handling(x_values, y_matrix, gap_gap_vals, ...
    x_label, y_label, title_str, save_path)
...
%----------------------------------------------------------
% (C) gap_gap で最大値 + シングル (2本線)
%    -> Single を青色，Double を赤色で明示的に指定
%       (プロットの順番も Single -> Double にして，
%        レジェンド順が Single(青)，Double(赤) になるように。)
%----------------------------------------------------------
function plot_max_over_gapgap_with_single(x_values, y_matrix, single_values, ...
    x_label, y_label, title_str, save_path, legend_dc, legend_sg)
figure('Name', title_str);
hold on; grid on;
...
% ★★ ここを修正: Single(青) → Double(赤) の順で描画します ★★
plot(x_values, single_values, '-o', 'Color', co(1, :), ...
    'DisplayName', legend_sg);    % Single = 青
plot(x_values, max_vals, '-o', 'Color', co(2, :), ...
    'DisplayName', legend_dc);    % Double = 赤
...
%   - Power Equivalent Value: G1_peq_2D, G2_peq_2D, G_sum_peq_2D
%   - Acceleration Gradient: G1_abs_2D, G2_abs_2D, G_abs_sums_2D
%--------------------------------------------------
...
plot_with_nan_handling( ...
    ...
    'Power Equivalent Value (channel 1) vs gap for each gap\_gap', ...
    fullfile(output_folder_name, ['peq_g1_vs_gap_for_each_gapgap_', timestamp_str, '.png']) ...
    );
plot_with_nan_handling( ...
    gap_nm_values, G2_peq_2D, gap_gap_nm_values, ...
    'gap (nm)', 'Power Equivalent Value (channel 2)', ...
    'Power Equivalent Value (channel 2) vs gap for each gap\_gap', ...
...
plot_with_nan_handling( ...
    ...
    'gap (nm)', 'Power Equivalent Value (Total)', ...
    'Power Equivalent Value (Total) vs gap for each gap\_gap', ...
    fullfile(output_folder_name, ['peq_gsum_vs_gap_for_each_gapgap_', timestamp_str, '.png']) ...
    );

%--------------------------------------------------
% (B) gap_gapで最大値 (ダブルチャネルのみ, 1本線)
%     -> レジェンド不要
%--------------------------------------------------
plot_max_over_gapgap( ...
    gap_nm_values, G1_peq_2D, ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (channel 1) vs gap (best gap\_gap)', ...
    fullfile(output_folder_name, ['max_peq_g1_vs_gap_', timestamp_str, '.png']) ...
    );
plot_max_over_gapgap( ...
    gap_nm_values, G2_peq_2D, ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (channel 2) vs gap (best gap\_gap)', ...
...
plot_max_over_gapgap( ...
    ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (Total) vs gap (best gap\_gap)', ...
    fullfile(output_folder_name, ['max_peq_gsum_vs_gap_', timestamp_str, '.png']) ...
    );

%--------------------------------------------------
% (C) gap_gapで最大値 + シングル (2本線)
%     -> Single=青，Double=赤
%--------------------------------------------------
plot_max_over_gapgap_with_single( ...
    gap_nm_values, G1_peq_2D, sG_peq_1D, ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (channel 1 & single) vs gap (best gap\_gap)', ...
...
plot_max_over_gapgap_with_single( ...
    ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (channel 2 & single) vs gap (best gap\_gap)', ...
...
plot_max_over_gapgap_with_single( ...
    ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (total & single) vs gap (best gap\_gap)', ...
...
plot_single_only( ...
    ...
    'gap (nm)', 'Power Equivalent Value', ...
    'Power Equivalent Value (single) vs gap', ...
