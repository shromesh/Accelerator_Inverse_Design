...
% ※ 内部的には 2000nm で割りたい場合はここで割る。
%    たとえば x = (abs(G)*gap_nm)/2000 のように計算しつつ、
%    ラベルには「/2000」や「Normalized」は書かない。

REF_nm = 2000;  % 必要なら 2000 nm で割るための定数
% 実際には "Power Equivalent Quantity" = abs(G)*gap/REF_nm とする例
...
%   - Power Equivalent Quantity: G1_peq_2D, G2_peq_2D, G_sum_peq_2D
%   - Acceleration Gradient: G1_abs_2D, G2_abs_2D, G_abs_sums_2D
%--------------------------------------------------

% (1) Power Equivalent Quantity (channel 1)
plot_with_nan_handling( ...
    gap_nm_values, ...
    G1_peq_2D, ...
    gap_gap_nm_values, ...
    'gap (nm)', ...
    'Power Equivalent Quantity (channel 1)', ...
...
% (2) Power Equivalent Quantity (channel 2)
plot_with_nan_handling( ...
    gap_nm_values, ...
    G2_peq_2D, ...
    gap_gap_nm_values, ...
    'gap (nm)', ...
    'Power Equivalent Quantity (channel 2)', ...
...
% (6) Power Equivalent Quantity (Total)
plot_with_nan_handling( ...
    gap_nm_values, ...
    G_sum_peq_2D, ...
    gap_gap_nm_values, ...
    'gap (nm)', ...
    'Power Equivalent Quantity (Total)', ...
...
% (1) Power Equivalent Quantity (ch1) + Single
...
% (2) Power Equivalent Quantity (channel 2) + Single
plot_max_over_gapgap_with_single( ...
    gap_nm_values, ...
    G2_peq_2D, ...
    sG_peq_1D, ...
...
% (3) Acceleration Gradient (channel 1) + Single
plot_max_over_gapgap_with_single( ...
    gap_nm_values, ...
    G1_abs_2D, ...
    sG_best_1D, ...
...
% (4) Acceleration Gradient (channel 2) + Single
plot_max_over_gapgap_with_single( ...
    gap_nm_values, ...
    G2_abs_2D, ...
    sG_best_1D, ...
...
% (5) Acceleration Gradient (Total) + Single
plot_max_over_gapgap_with_single( ...
    gap_nm_values, ...
    G_abs_sums_2D, ...
    sG_best_1D, ...
...
% (6) Power Equivalent Quantity (Total) + Single
plot_max_over_gapgap_with_single( ...
    gap_nm_values, ...
    G_sum_peq_2D, ...
    sG_peq_1D, ...
